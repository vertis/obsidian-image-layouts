...
const renderLayout = (
	...
	const div = parent.createEl("div", {
		cls: `image-layouts-grid image-layouts-layout-${layout}`,
	});

	images.forEach((image, idx) => {
		const imgdiv = div.createEl("div", {
			cls: `image-layouts-image-${idx}`,
		});
		if (image.type === "local") {
...
const renderMasonryLayout = (
	...
	const div = parent.createEl("div", {
		cls: `image-layouts-masonry-grid-${columns}`,
	});

	// create the an array of divs to hold the images
	const columnDivs: HTMLElement[] = [];
	for (let i = 0; i < columns; i++) {
		const colDiv = div.createEl("div", {
			cls: `image-layouts-masonry-column`,
		});
	...
	images.forEach((image, idx) => {
		...
		const imgdiv = columnDivs[colIdx].createEl("div", {
			cls: `image-layouts-masonry-image-${idx}`,
		});
		if (image.type === "local") {
			addImageFromLink(image.link, sourcePath, imgdiv, plugin);
		} else if (image.type === "external") {
