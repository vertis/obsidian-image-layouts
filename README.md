...
Images can either be in wikilink format `![[imagename]]` (with optional caption and size, e.g. `![[imagename|My caption]]`) in which case it will load from the local vault, or in `![](url)` format, which will load either the local or remote image as applicable.
...
See the [Block Options Reference](https://github.com/vertis/obsidian-image-layouts/blob/main/docs/options.md) for everything a layout block accepts.
...
- [Block Options Reference](https://github.com/vertis/obsidian-image-layouts/blob/main/docs/options.md) - All options for every layout block: captions, overlays, fit, alignment, folder galleries, carousel options.
